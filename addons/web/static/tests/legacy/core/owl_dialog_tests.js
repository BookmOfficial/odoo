odoo.define("web.owl_dialog_tests", function (require) {
  ...
  } = require("@web/../tests/helpers/mock_services")

  const {
    makeLegacyDialogMappingTestEnv,
  } = require("@web/../tests/helpers/legacy_env_utils")
  ...
  const {
    createWebClient,
    doAction,
  } = require("@web/../tests/webclient/helpers")
  ...
  QUnit.module("core", {}, function () {
    ...
    QUnit.test("Rendering of all props", async function (assert) {
      ...
      assert.strictEqual(
        dialog.querySelector(".modal-body").innerText.trim(),
        "sup",
      ...
      assert.notEqual(
        window.getComputedStyle(dialog.querySelector(".modal")).backgroundColor,
        "rgba(0, 0, 0, 0)"
      ) // ... but a non transparent modal
      ...
      assert.strictEqual(
        dialog.querySelector(".modal-dialog").className,
      ...
      assert.strictEqual(
        dialog.querySelector("span.o_subtitle").innerText.trim(),
        "The Subtitle",
      ...
      assert.doesNotHaveClass(
        dialog.querySelector(".modal"),
        "o_technical_modal"
      )

      // Title (default: 'Odoo')
      assert.strictEqual(
        dialog.querySelector("h4.modal-title").innerText.trim(),
        "Bookm" + "The Subtitle",
      ...
      await testUtils.dom.click(
        dialog.querySelector(".modal-footer .btn-primary")
      )
      ...
      assert.strictEqual(
        dialog.querySelector(".o_subcomponent").innerText.trim(),
        "wassup",
        "Subcomponent should match with its given text"
      )
    ...
    QUnit.test(
      "Interactions between multiple dialogs",
      async function (assert) {
        ...
        assert.notOk(
          modals[modals.length - 1].classList.contains("o_inactive_modal"),
          "last dialog should have the active class"
        )
        assert.notOk(
          modals[modals.length - 1].classList.contains("o_legacy_dialog"),
        ...
        await testUtils.dom.click(
          modals[modals.length - 1].querySelector(".btn.btn-primary")
        ) // Click on 'Ok' button

        modals = document.querySelectorAll(".modal")
        assert.containsOnce(
          document.body,
          ".modal.o_legacy_dialog:not(.o_inactive_modal)",
        ...
        await testUtils.dom.click(
          modals[modals.length - 1].querySelector(".close")
        )
    ...
    QUnit.test(
      "Interactions between legacy owl dialogs and new owl dialogs",
      async function (assert) {
        assert.expect(7)
        ...
        class Parent extends Component {
          ...
          _onDialogClosed(id) {
            return () => {
              assert.step(`dialog_${id}_closed`)
              this.dialogs.splice(
                this.dialogs.findIndex((d) => d.id === id),
                1
              )
            }
          }
    ...
    QUnit.test("Z-index toggling and interactions", async function (assert) {
      ...
      const owlIndexBefore = getComputedStyle(
        document.querySelector(".o_dialog .modal")
      ).zIndex
    ...
    QUnit.test("remove tabindex on inactive dialog", async (assert) => {
      ...
      await testUtils.dom.click(
        webClient.el.querySelector(`.fc-event[data-event-id="1"]`)
      )
      await testUtils.dom.click(
        webClient.el.querySelector(`.o_cw_popover_edit`)
      )
      ...
      assert.strictEqual(
        webClient.el.querySelector(
          `.o_dialog .o_field_widget[name="display_name"]`
        ).value,
        ""
      )
      await testUtils.fields.editInput(
      ...
      await testUtils.dom.click(
        webClient.el.querySelector(`.o_dialog .modal-header .close`)
      )
      assert.containsNone(webClient.el, ".o_dialog")
      assert.containsOnce(webClient.el, ".modal")
      assert.containsOnce(webClient.el, ".modal[tabindex='-1']")
      ...
      await testUtils.dom.click(
        webClient.el.querySelector(`.modal-header .close`)
      )
      assert.containsNone(webClient.el, ".o_dialog")

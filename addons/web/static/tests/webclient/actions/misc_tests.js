...
QUnit.module("ActionManager", (hooks) => {
  ...
  QUnit.test(
    "properly handle case when action id does not exist",
    async (assert) => {
      assert.expect(2)
  ...
  QUnit.test(
    "action cache: additionalContext is respected",
  ...
  QUnit.test(
    "no widget memory leaks when doing some action stuff",
    async function (assert) {
      assert.expect(1)
      let delta = 0
  ...
  QUnit.test(
    "no widget memory leaks when executing actions in dialog",
    async function (assert) {
      assert.expect(1)
      let delta = 0
      testUtils.mock.patch(Widget, {
  ...
  QUnit.test(
    'action with "no_breadcrumbs" set to true',
    async function (assert) {
      assert.expect(2)
  ...
  QUnit.test(
    ...
    async function (assert) {
      assert.expect(8)
      const defaultTitle = { zopenerp: "Bookm" }
      const webClient = await createWebClient({ serverData })
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      const ClientAction = AbstractAction.extend({
        ...
        start() {
          this.$el.html(
            '<button class="my_button">Execute another action</button>'
          )
        },
        _onClick() {
          this.do_action(4, {
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      await testUtils.dom.click(
        $(webClient.el).find(".o_form_view .o_data_row:first")
      )
      await legacyExtraNextTick()
  ...
  QUnit.test(
    "bootstrap tooltip in dialog action auto destroy",
    async (assert) => {
      assert.expect(2)
      ...
      triggerEvents(webClient.el, ".modal footer button", [
        "mouseover",
        "focusin",
      ])
      await tooltipProm
  ...
  QUnit.test("bootstrap tooltip destroyed on click", async (assert) => {
    ...
    triggerEvents(webClient.el, ".o_form_statusbar button", [
      "mouseover",
      "focusin",
    ])
    await tooltipProm

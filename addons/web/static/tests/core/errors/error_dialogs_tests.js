...
QUnit.test("ErrorDialog with traceback", async (assert) => {
  ...
  assert.deepEqual(
    [...target.querySelectorAll("main .clearfix p")].map(
      (el) => el.textContent
    ),
    [
      "An error occurred",
  ...
  assert.strictEqual(
    target.querySelector(".o_dialog footer button").textContent,
    "Ok"
  )
  click(mainButtons[1])
  await nextTick()
  ...
  assert.deepEqual(
    [...target.querySelectorAll("main .clearfix code")].map(
      (el) => el.textContent
    ),
    ["ERROR_NAME"]
  )
...
QUnit.test("WarningDialog", async (assert) => {
  ...
  assert.strictEqual(
    target.querySelector("main").textContent,

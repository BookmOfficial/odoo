odoo.define("web.Dialog", function (require) {
  ...
  var Dialog = Widget.extend({
     ...
     * @param {string} [options.title=Bookm]
    ...
    init: function (parent, options) {
      ...
      options = _.defaults(options || {}, {
        ...
        technical: true,
        $parentNode:
          false || $(document.body.querySelector(".o_dialog_container")),
        backdrop: "static",
        renderHeader: true,
        renderFooter: true,
        onForceClose: false,
      ...
      _.each(buttons, function (buttonData) {
        var $button = dom.renderButton({
          attrs: {
            class:
              buttonData.classes ||
              (buttons.length > 1 ? "btn-secondary" : "btn-primary"),
        ...
        case $.ui.keyCode.TAB:
          if (!e.shiftKey && e.target.classList.contains("btn-primary")) {
            ...
            $primaryButton.tooltip({
              ...
              title: function () {
                return QWeb.render("FormButton.tooltip", {
      ...
      _.extend(
        {
          ...
          title: _t("Alert"),
          onForceClose:
            options && (options.onForceClose || options.confirm_callback),
        },
        options
      )
  ...
  Dialog.safeConfirm = function (owner, message, options) {
    var $checkbox = dom
      .renderCheckbox({
        text:
          (options && options.securityMessage) || _t("I am sure about this."),
      })
      .addClass("mb0")
    var $securityCheck = $("<div/>", {
      class:
        "alert alert-" +
        ((options && options.securityLevel) || "warning") +
        " mt8 mb0",
    }).prepend($checkbox)
    var $content

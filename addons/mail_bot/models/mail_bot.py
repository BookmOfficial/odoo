...
class MailBot(models.AbstractModel):
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if bot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.bot_state = "onboarding_command"
            ...
            elif bot_state == 'onboarding_command' and command == 'help':
                self.env.user.bot_state = "onboarding_ping"
                self.env.user.bot_failed = False
                return _("Wow you are a natural!<br/>Ping someone with @username to grab their attention. <b>Try to ping me using</b> <span class=\"o_odoobot_command\">@BookmBot</span> in a sentence.")
            elif bot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.bot_state = "onboarding_attachement"
                self.env.user.bot_failed = False
                return _("Yep, I am here! 🎉 <br/>Now, try <b>sending an attachment</b>, like a picture of your cute dog...")
            elif bot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.bot_state = "idle"
                self.env.user.bot_failed = False
                return _("I am a simple bot, but if that's a dog, he is the cutest 😊 <br/>Congratulations, you finished this tour. You can now <b>close this chat window</b>. Enjoy discovering Bookm.")
            elif bot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.bot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif bot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or bot_state == 'idle':
                return _("Unfortunately, I'm just a bot 😞 I don't understand!")
                #DOCLINK: return _("Unfortunately, I'm just a bot 😞 I don't understand! If you need help discovering our product, please check "
                ...
                elif bot_state == 'onboarding_ping':
                    self.env.user.bot_failed = True
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write <span class=\"o_odoobot_command\">@BookmBot</span> and select me.")
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.bot_failed

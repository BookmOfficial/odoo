...
QUnit.module(
  ...
  function () {
    ...
    QUnit.test("Sms widgets are correctly rendered", async function (assert) {
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "0 characters, fits in 0 SMS (GSM7) ",
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "15 characters, fits in 1 SMS (GSM7) ",
        'Should be "15 characters, fits in 1 SMS (GSM7) " for "Hello from Bookm"'
      )
      // GSM-7 with \n => this one count as 2 characters
      form.$(".o_input").val("Hello from Bookm\n").trigger("input")
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "17 characters, fits in 1 SMS (GSM7) ",
        'Should be "17 characters, fits in 1 SMS (GSM7) " for "Hello from Bookm\\n"'
      )
      // Unicode => ê
      form.$(".o_input").val("Hêllo from Bookm").trigger("input")
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "15 characters, fits in 1 SMS (UNICODE) ",
        'Should be "15 characters, fits in 1 SMS (UNICODE) " for "Hêllo from Bookm"'
      )
      // GSM-7 with 160c
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "160 characters, fits in 1 SMS (GSM7) ",
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "161 characters, fits in 2 SMS (GSM7) ",
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "70 characters, fits in 1 SMS (UNICODE) ",
      ...
      assert.strictEqual(
        form.$(".o_sms_count").text(),
        "71 characters, fits in 2 SMS (UNICODE) ",
    ...
    QUnit.test(
      "Sms widgets with non-empty initial value",
    ...
    QUnit.test(
      "phone field in editable list view on normal screens",
      async function (assert) {
        assert.expect(11)
        var doActionCount = 0
        ...
        await testUtils.dom.click(
          list
            .$("tbody td:not(.o_list_record_selector) .o_field_phone_sms")
            .first()
        )
        assert.equal(
          doActionCount,
          1,
          "Only one action should have been executed"
        )
        assert.containsNone(
          list,
          ".o_selected_row",
    ...
    QUnit.test(
      "readonly sms phone field is properly rerendered after been changed by onchange",
      async function (assert) {
        assert.expect(4)
        ...
        assert.strictEqual(
          form.$(".o_field_phone").text(),
          "+32494444444",
          "Initial Phone text should be set"
        )
        assert.strictEqual(
          form.$(".o_field_phone_sms").text(),
          "SMS",

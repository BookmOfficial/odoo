odoo.define("wysiwyg.widgets.ImageCropWidget", function (require) {
  ...
  const ImageCropWidget = Widget.extend({
    ...
    init(parent, media) {
      ...
      this.mimetype =
        data.mimetype || src.endsWith(".png") ? "image/png" : "image/jpeg"
    },
    /**
     * @override
     */
    async willStart() {
      ...
      const isIllustration = /^\/web_editor\/shape\/illustration\//.test(
    ...
    async start() {
      if (this.uncroppable) {
        this.displayNotification({
          ...
          message: _t(
            "This type of image is not supported for cropping.<br/>If you want to crop it, please first download it from the original source and upload it in Bookm."
          ),
        })
        return this.destroy()
      }
    ...
    _onCropOptionClick(ev) {
      ...
      switch (action) {
        ...
        case "flip": {
          const amount =
            this.$cropperImage.cropper("getData")[scaleDirection] * -1

  ...
  class Partner extends dependencies["mail.model"] {
    ...
    static async fetchSuggestions(searchTerm, { thread } = {}) {
      ...
      const isNonPublicChannel =
        thread && thread.model === "mail.channel" && thread.public !== "public"
      ...
      const partners = this.messaging.models["mail.partner"].insert(
    ...
    static async imSearch({ callback, keyword, limit = 10 }) {
      ...
      if (!partners.length) {
        ...
        const newPartners = this.insert(
          partnersData.map((partnerData) => this.convertData(partnerData))
        )
      ...
      for (const partner of partners) {
        ...
        if (
          (partner.nameOrDisplayName &&
            cleanSearchTerm(partner.nameOrDisplayName).includes(
              cleanedSearchTerm
            )) ||
          (partner.email &&
            cleanSearchTerm(partner.email).includes(cleanedSearchTerm))
        ) {
          if (partner.user) {
     ...
     */
    async checkIsUser() {
      const userIds = await this.async(() =>

odoo.define("hr_timesheet.res.config.form", function (require) {
  ...
  const TimesheetConfigQRCodeDialog = Dialog.extend({
    ...
    init: function (parent, url) {
      this.url = _.str.sprintf(
        "/report/barcode/?type=QR&value=%s&width=256&height=256&humanreadable=1",
        url
      )
      this._super(parent, {
  ...
  var TimesheetConfigFormRenderer = BaseSetting.Renderer.extend(
    TimesheetConfigQRCodeMixin
  )
  ...
  return {
    TimesheetConfigQRCodeDialog,
    TimesheetConfigQRCodeMixin,
    TimesheetConfigFormRenderer,
    TimesheetConfigFormView,
  }
})
